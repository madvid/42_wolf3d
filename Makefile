...
#    Updated: 2021/05/15 21:06:45 by mdavid           ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
INC_WOLF= includes/wolf.h includes/wolfdata.h
INC_SERV= includes/server.h
SRC_NAME= main.c events.c tools.c camera.c parser.c misc.c draw_scene.c\
			...
			receive_players_pos.c parser_check_file.c
...
.PHONY: all clean fclean re local install
...
server: obj/init_socket.o obj/server.o obj/srv_connect_client.o\
	obj/srv_receive.o obj/srv_remove_client.o obj/srv_send_data.o $(INC_SERV)
	make -C libft
...
$(OBJ_PATH)%.o: $(SRC_PATH)%.c $(INC_WOLF)
	@mkdir $(OBJ_PATH) 2> /dev/null || true
	$(CC) $(CFLAGS) $(INC) -o $@ -c $<
